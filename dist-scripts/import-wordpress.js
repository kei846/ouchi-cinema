...
const block_tools_1 = require("@portabletext/block-tools");
...
const defaultSchema = schema_1.default.compile({
    name: 'myBlog',
    types: [
        {
            type: 'object',
...
const blockContentType = (_a = defaultSchema.get('blogPost').fields.find((field) => field.name === 'body')) === null || _a === void 0 ? void 0 : _a.type;
if (!blockContentType) {
    throw new Error('Could not find block content type in schema.');
}
const WP_BASE_URL = 'https://mikomikopipi.com/wp-json/wp/v2';
async function fetchWordPressData(endpoint) {
...
async function htmlToPortableText(html) {
    ...
    return (0, block_tools_1.htmlToBlocks)(html, blockContentType, {
        parseHtml: (htmlString) => new jsdom_1.JSDOM(htmlString).window.document,
    });
}
async function importCategories(wpCategories) {
    ...
    for (const wpCategory of wpCategories) {
        ...
        await sanityClient_1.sanityClient.createOrReplace(doc);
...
async function importPosts() {
    ...
    const wpCategories = await fetchWordPressData('categories');
    const categoryMap = await importCategories(wpCategories);
    console.log('Fetching WordPress tags...');
    const wpTags = await fetchWordPressData('tags');
    const tagMap = await importTags(wpTags);
    console.log('Fetching WordPress posts...');
    const wpPosts = await fetchWordPressData('posts');
